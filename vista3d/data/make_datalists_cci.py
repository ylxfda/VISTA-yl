...
@register_make
def make_CCI50():
    base_url = "/data/DataSSD3/yli/data/nnunet_raw/nnunet_cranialid/Dataset011_cranialid/"
    dataset_name = "CranialID50"
    json_name = os.path.join(output_json_dir, f"{dataset_name}_{n_folds}_folds.json")
    masks = search_image_files(os.path.join(base_url, "labelsTr"), ".nrrd")
    # masks += search_image_files(os.path.join(base_url, "labelsVa"), ".nii.gz")
    masks += search_image_files(os.path.join(base_url, "labelsTs"), ".nrrd")
    images, labels = [], []
    for mask in masks:
        rel_mask = os.path.relpath(mask, base_url)
        labels.append(rel_mask)
        img_name = rel_mask.split("/")[1].replace(".nrrd", "_0000.nrrd")
        for f in ["imagesTr", "imagesTs"]:
